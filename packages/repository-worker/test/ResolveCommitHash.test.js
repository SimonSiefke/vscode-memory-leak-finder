...
test('resolveCommitHash - returns commitRef when it is already a full commit hash', async () => {
  ...
  expect(mockExecStdout).not.toHaveBeenCalled()
})
...
test('resolveCommitHash - resolves branch name to commit hash', async () => {
  ...
  mockExecStdout.mockImplementation(() => Promise.resolve(mockStdout))
  ...
  expect(mockExecStdout).toHaveBeenCalledTimes(1)
  expect(mockExecStdout).toHaveBeenCalledWith('git', ['ls-remote', 'https://github.com/test/repo.git', 'main'])
})

test('resolveCommitHash - throws error when no commit found', async () => {
  mockExecStdout.mockImplementation(() => Promise.resolve(''))
  ...
  expect(mockExecStdout).toHaveBeenCalledTimes(1)
  expect(mockExecStdout).toHaveBeenCalledWith('git', ['ls-remote', 'https://github.com/test/repo.git', 'nonexistent-branch'])
})

test('resolveCommitHash - throws error when git ls-remote fails', async () => {
  mockExecStdout.mockImplementation(() => Promise.reject(new Error('Repository not found')))
...
test('resolveCommitHash - resolves tag to commit hash', async () => {
  ...
  expect(mockExecStdout).toHaveBeenCalledTimes(1)
  expect(mockExecStdout).toHaveBeenCalledWith('git', ['ls-remote', 'https://github.com/test/repo.git', 'v1.0.0'])
})
...
test('resolveCommitHash - handles different repository URLs', async () => {
  ...
  expect(mockExecStdout).toHaveBeenCalledTimes(1)
  expect(mockExecStdout).toHaveBeenCalledWith('git', ['ls-remote', 'https://gitlab.com/test/repo.git', 'feature'])
})
...
test('resolveCommitHash - handles short commit hash input', async () => {
  ...
  // Should call execStdout since it's not a full 40-character hash
  expect(mockExecStdout).toHaveBeenCalledTimes(1)
  expect(mockExecStdout).toHaveBeenCalledWith('git', ['ls-remote', 'https://github.com/test/repo.git', shortHash])
})
