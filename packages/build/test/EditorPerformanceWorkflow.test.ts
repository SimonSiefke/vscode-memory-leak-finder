...
test('editor performance workflow measures both revisions in every replica', async () => {
  ...
  expect(workflow).toContain("--track-include 'vs/base/browser/ui/breadcrumbs/breadcrumbsWidget.ts'")
  expect(workflow).toContain("--track-include 'vs/editor/browser/config/editorConfiguration.ts'")
  expect(workflow).toContain("--track-include 'vs/editor/browser/config/migrateOptions.ts'")
  expect(workflow).toContain("--track-include 'vs/workbench/contrib/files/browser/editors/textFileEditor.ts'")
