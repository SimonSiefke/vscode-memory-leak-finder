...
function getIncompleteReason(complete: boolean, dataLossOccurred: boolean, dumpCount: number): string {
  if (complete) {
    return ''
  }
  if (dataLossOccurred) {
    return 'Chromium reported trace data loss'
  }
  return `Expected exactly two detailed Chromium memory dumps, received ${dumpCount}`
}
...
export const createChromiumMemoryDumpResult = (
  ...
  return {
    ...
    unsupportedReason: getIncompleteReason(complete, dataLossOccurred, dumps.length),
  }
}
