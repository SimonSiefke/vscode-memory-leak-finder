  ...
  for (const workerSession of workerSessions) {
    ...
    if (target) {
      ...
      if (measureClass) {
        ...
        results[workerName] = await workerMeasure.stop()
      }
    }
  }
...
export const stopWithWorkers = async (instanceId: string, electronTargetId: string): Promise<any> => {
  const crashInfo = WaitForCrash.waitForCrash(electronTargetId)
  const resultPromise = doStopWithWorkers(instanceId)
