...
export const downloadAndUnzipInsiders = async (commit: string): Promise<string> => {
  ...
  if (cachedPath) {
    console.log(`[download-worker] [insiders] Using cached vscode-insiders for commit ${commit}`)
    return cachedPath
  }
  console.log(`[download-worker] [insiders] Cache miss, downloading vscode-insiders for commit ${commit}`)
  ...
  console.log(`[download-worker]`, { url: metadata.url })
