...
- Handles `pull_request.synchronize` events by rerunning the latest valid command from an allowed user on that pull request.
...
| `BOT_ALL_MOCK_DATA_R2_OBJECT_KEY`    | No       | `all-mock-data.zip`         | Object key for the uploaded archive that bundles `.vscode-user-data-dir`, `.vscode-mock-requests`, `.vscode-proxy-certs`, `.vscode-sse-data`, and `.vscode-requests`. |
...
- `BOT_ALL_MOCK_DATA_R2_OBJECT_KEY`
- `BOT_PUBLIC_BASE_URL`
- `BOT_USER_DATA_UPLOAD_TOKEN`

`BOT_ALL_MOCK_DATA_R2_OBJECT_KEY` is optional because it defaults to `all-mock-data.zip`.
