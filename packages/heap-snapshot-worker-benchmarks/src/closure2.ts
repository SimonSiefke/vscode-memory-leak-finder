...
const { compareNamedClosureCountFromHeapSnapshot2 } = await importHeapSnapshotWorker(
  'src/parts/CompareNamedClosureCount2/CompareNamedClosureCount2.ts',
)
...
const testOptimized = async () => {
  ...
  try {
    ...
    const values = await compareNamedClosureCountFromHeapSnapshot2(filePath1, filePath2)
