...
export const launchIde = async ({ headlessMode, cwd, ide, vscodePath, commit }) => {
  if (ide === Ide.Cursor) {
    const cursorVersion = '0.45.14' // TODO make it configurable
    return LaunchCursor.launchCursor({ headlessMode, cwd, cursorVersion, vscodePath })
  }
  return LaunchVsCode.launchVsCode({ headlessMode, cwd, vscodeVersion: VscodeVersion.vscodeVersion, vscodePath, commit })
}
