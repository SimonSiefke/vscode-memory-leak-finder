...
import { parseHeapSnapshotArrayHeader } from '../src/parts/ParseHeapSnapshotArrayHeader/ParseHeapSnapshotArrayHeader.ts'
...
test('parseHeapSnapshotArrayHeader - finds nodes array header', () => {
  ...
  expect(result).toBe(9) // Position after '['
})
...
test('parseHeapSnapshotArrayHeader - handles whitespace around token', () => {
  ...
  expect(result).toBe(-1) // Function doesn't handle whitespace
})
...
test('parseHeapSnapshotArrayHeader - handles data with prefix', () => {
  ...
  expect(result).toBe(24) // Position after '['
})
...
test('parseHeapSnapshotArrayHeader - handles multiple occurrences of token', () => {
  ...
  expect(result).toBe(9) // Should find the first occurrence
})
...
test('parseHeapSnapshotArrayHeader - handles token with special characters', () => {
  ...
  expect(result).toBe(15) // Position after '['
})
