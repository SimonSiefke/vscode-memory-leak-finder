...
<html lang="en">
  <head>
    ...
    <title>React Vite Checkbox Fixture</title>
  </head>
  <body>
    <div id="root"></div>
...
const getAppSource = (): string => {
  return `const collapsedCheckboxCount = ${collapsedCheckboxCount}
const expandedCheckboxCount = ${expandedCheckboxCount}

export const CheckboxCounter = ({ React }) => {
  const [checkboxCount, setCheckboxCount] = React.useState(collapsedCheckboxCount)
  const toggleCheckboxCount = () => {
    setCheckboxCount((count) => (count === collapsedCheckboxCount ? expandedCheckboxCount : collapsedCheckboxCount))
  }
  ...
  return React.createElement(
    'main',
    undefined,
    React.createElement('h1', undefined, 'Checkbox Fixture'),
    React.createElement(
      'button',
      {
        id: 'toggle-checkbox-count',
        type: 'button',
        onClick: toggleCheckboxCount,
      },
      checkboxCount === collapsedCheckboxCount ? 'Show more checkboxes' : 'Show fewer checkboxes',
    ),
    React.createElement(
      'p',
      {
...
createRoot(root).render(React.createElement(CheckboxCounter, { React }))
`
}
