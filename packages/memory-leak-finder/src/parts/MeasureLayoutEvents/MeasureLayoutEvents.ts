...
import * as LayoutEvents from '../LayoutEvents/LayoutEvents.ts'
...
export const id = MeasureId.LayoutEvents
...
export const start = async (session: Session, state: { trace: TraceState }) => {
  ...
  return LayoutEvents.getLayoutEvents([])
}
...
export const stop = async (session: Session, state: { trace: TraceState }) => {
  ...
  return LayoutEvents.getLayoutEvents(state.trace.events, state.trace.dataLossOccurred)
}
...
export const summary = ({ events, metrics }: Dynamic) => {
  return LayoutEvents.formatLayoutEventsSummary({ events, metrics, rawEvents: [] })
}
