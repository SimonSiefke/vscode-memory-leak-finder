...
export const id = MeasureId.DuplicatedStrings
...
export const compare = async (beforePath: string, afterPath: string): Promise<DuplicatedStringsComparison> => {
  await using rpc = await launchHeapSnapshotWorker()
  return await HeapSnapshotFunctions.compareDuplicatedStrings(rpc, beforePath, afterPath)
}

export const isLeak = ({ after, before }: DuplicatedStringsComparison): boolean => {
