...
} from '../JavascriptExecutionTime/JavascriptExecutionTime.ts'
import * as MeasureId from '../MeasureId/MeasureId.ts'
import * as TargetId from '../TargetId/TargetId.ts'
import { DevtoolsProtocolPerformance } from '../DevtoolsProtocol/DevtoolsProtocol.ts'

export const id = MeasureId.JavascriptExecutionTime
...
export const compare = (before: Dynamic, after: Dynamic) => {
  const metrics = normalizeJavascriptExecutionTime(before, after)
  return {
    isLeak: false,
    metrics,
    raw: {
...
export const summary = ({ metrics }: Dynamic) => {
  return formatJavascriptExecutionTimeSummary(metrics)
}
