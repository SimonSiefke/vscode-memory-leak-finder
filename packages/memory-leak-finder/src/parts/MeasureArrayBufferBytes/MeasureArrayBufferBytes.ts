...
interface ArrayBufferBytesState {
  readonly afterPath: string
  readonly beforePath: string
  captured: boolean
}

export const id = MeasureId.ArrayBufferBytes
export const targets = [TargetId.Browser, TargetId.Node, TargetId.Worker]

export const create = (session: Session) => {
  const captureId = randomUUID()
  const beforePath = join(Root.root, '.vscode-heapsnapshots', `array-buffer-bytes-${captureId}-before.json`)
  const afterPath = join(Root.root, '.vscode-heapsnapshots', `array-buffer-bytes-${captureId}-after.json`)
  process.once('exit', () => {
    rmSync(beforePath, { force: true })
    rmSync(afterPath, { force: true })
  })
  return [session, { afterPath, beforePath, captured: false } satisfies ArrayBufferBytesState] as const
}

export const start = async (session: Session, state: ArrayBufferBytesState): Promise<string> => {
  await HeapSnapshot.takeHeapSnapshot(session, state.beforePath)
  return state.beforePath
}

export const stop = async (session: Session, state: ArrayBufferBytesState): Promise<string> => {
...
export const compare = async (beforePath: string, afterPath: string): Promise<Dynamic> => {
  try {
    ...
    return await rpc.invoke('HeapSnapshot.compareArrayBufferBytes', beforePath, afterPath)
  } finally {
...
export const releaseResources = async (_session: Session, state: ArrayBufferBytesState): Promise<void> => {
  if (!state.captured) {
