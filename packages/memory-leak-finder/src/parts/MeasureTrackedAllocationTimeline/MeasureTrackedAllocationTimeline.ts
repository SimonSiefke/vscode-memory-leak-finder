...
import * as CompareTrackedAllocationTimeline from '../CompareTrackedAllocationTimeline/CompareTrackedAllocationTimeline.ts'
...
export const id = MeasureId.TrackedAllocationTimeline
...
export const runCompletion = async (session: Session) => {
  await GetTrackedAllocations.markTrackedAllocationRun(session)
}

export const stop = async (session: Session, scriptHandler: IScriptHandler) => {
  await ForceGarbageCollection.forceGarbageCollection(session)
  const runs = await GetTrackedAllocations.getTrackedAllocationRuns(session)
...
export const compare = CompareTrackedAllocationTimeline.compareTrackedAllocationTimeline
