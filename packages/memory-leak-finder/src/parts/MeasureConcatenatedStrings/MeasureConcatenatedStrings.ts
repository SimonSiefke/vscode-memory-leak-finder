...
export const id = MeasureId.ConcatenatedStrings
...
export const compare = async (beforePath: string, afterPath: string): Promise<ConcatenatedStringsComparison> => {
  await using rpc = await launchHeapSnapshotWorker()
  return await HeapSnapshotFunctions.compareConcatenatedStrings(rpc, beforePath, afterPath)
}

export const isLeak = ({ after, before }: ConcatenatedStringsComparison): boolean => {
